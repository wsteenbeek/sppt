...
#' Performs Andresen's Spatial Point Pattern Test. (Global, Robust, Generalized)
#'
...
#'
#' set.seed(76772)
#' myoutput <- sppt_new(base_points.sp = points1.sp, test_points.sp = points2.sp, uoa.sp = areas.sp)
#' summary_sppt(myoutput)
#' myoutput@data
#'
#' # is exactly the same as:
#' set.seed(76772)
#' myoutput <- sppt_new(points1.sp, points2.sp, areas.sp,
...
#' set.seed(76772)
#' myoutput <- sppt_new(points1.sp, points2.sp, areas.sp, outputlist = TRUE)
#' head(myoutput[[1]]@data) # same as previous
#' head(myoutput[[2]])
#'
#' # Vancouver data
#' set.seed(9866)
#' myoutput <- sppt_new(vancouver_points1.sp, vancouver_points2.sp, vancouver_areas.sp)
#' summary_sppt(myoutput)
#'
#' # Vancouver data, using a bootstrap approach instead of Andresen's subsampling method
#' set.seed(9866)
#' myoutput <- sppt_new(vancouver_points1.sp, vancouver_points2.sp, vancouver_areas.sp, bootstrap = TRUE)
#' summary_sppt(myoutput)
#'
#' @keywords internal
sppt_new <- function(base_points.sp, test_points.sp, uoa.sp, outputlist = FALSE, nsamples = 200, percpoints = 85, conf_level = 95, bootstrap = FALSE){
