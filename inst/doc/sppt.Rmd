...
Though conceptually simple, this test is simulation-based so it is computationally intense. The test is computed in this R library as follows:
...
The standard and robust global S-values can be extracted as such, respectively: `mean(toy.sp$globalS)` and `mean(toy.sp$globalS.robust, na.rm=TRUE)`. A convenience function is provided to output these values:
...
In order to import your own spatial data into R, the library `rgdal` will need to be installed. In the next example, residential burglary data from the Vancouver Police Department (2003 and 2016) are used, as well as census boundary units (dissemination areas) from Statistics Canada.
...
The output of the sppt can be easily mapped, here simply using the `plot` command (but see the [ggplot2](https://CRAN.R-project.org/package=ggplot2) library):
