---
title: "Spatial Point Pattern Test (SPPT): improved functions as per Wheeler et al. (2018)"
...
`sppt_boot()` diffes from the original `sppt()` in that it does a resampling procedure on both Base and Test data. This function does NOT simply calculate a confidence interval around Base (like for Test in the `sppt()` function) and then inspects the overlap of the two confidence intervals. Instead, it takes a random sample from Base and Test for each areal unit, calculates the proportion of samples Base and Test points in each areal unit, and then the difference between the two percentages is calculated immediately (and this is done nsamples times for all areal units). If the 95% distribution of the differences between (resampled) Base and Test percentages excludes 0, they are deemed statistically different from each other.
...
As opposed to the traditional SPPT, this test calculates the difference in the proportions using either a Chi-square proportions test or Fisher's exact test. If required, the p-values are then adjusted for multiple comparisons.
...
Using the default values for sppt(), sppt_boot(), and Chi-Square with Yates's correction without p-value adjustment for multiple comparisons:
...
Any difference between the two point patterns is due to random variation, and so we'd like to see the globalS values to be quite high, indicating stability. We use the default values for sppt(), sppt_boot(), and Chi-Square with Yates's correction without p-value adjustment for multiple comparisons:
